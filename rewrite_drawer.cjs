          ...
          <div className="fixed inset-0 z-50 overflow-hidden select-none">
            ...
            >
              ...
              <div className="p-5 sm:p-6 space-y-5 relative z-10 flex-1 overflow-y-auto no-scrollbar">
                ...
                {/* Top Spotify-Style Account Area */}
                <div className="flex flex-col gap-3 pt-2 pb-2">
                  <div
                    onClick={() => setActiveSubModal('accounts')}
                    className="flex items-center gap-3 w-full cursor-pointer group active:scale-[0.98] transition-transform"
                  >
                    <UserAvatar
                      avatarUrl={profile?.avatar}
                      name={userName}
                      sizeClassName="w-13 h-13 shadow-md ring-1 ring-white/10 flex-shrink-0"
                      iconClassName="w-7 h-7 text-neutral-300"
                    />

                    <div className="flex flex-col min-w-0 flex-1 justify-center">
                      <h2 className="text-xl font-bold text-white tracking-tight truncate leading-tight group-hover:text-[#1ed760] transition-colors">
                        {userName}
                      </h2>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {savedAccounts && savedAccounts.filter(a => a.email !== firebaseUser?.email).slice(0, 3).map((acc, idx) => (
                        <div key={idx} className="w-8 h-8 rounded-full overflow-hidden border border-white/20 shadow-sm flex items-center justify-center bg-zinc-700 text-xs font-bold text-white uppercase" style={{ backgroundColor: ['#E0B0FF', '#FFB6C1', '#87CEFA', '#98FB98'][idx % 4] }}>
                          {acc.avatar ? (
                      ...
                      <button 
                        ...
                        className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white border border-white/20 transition-colors"
                      >
                    ...
                    recentItems.map(({ track, playedAt }, idx) => (
                      ...
                      >
                        <div className="w-11 h-11 rounded-xl overflow-hidden bg-neutral-800 flex-shrink-0 shadow-sm ring-1 ring-white/10 flex items-center justify-center">
                          {track.images.small || track.images.medium ? (
                            <img
...
console.log('Successfully re-wrote ProfileDrawer.tsx');
