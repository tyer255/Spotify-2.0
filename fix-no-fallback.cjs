...
const newResolve = `const resolveMusic = async (detectedTag: SpotifyTag, frame: HTMLCanvasElement | null) => {
    ...
    // 3. Prevent raw octal sequence search. If we failed to find any text or mapped track,
    // we MUST NOT search for "7727..." and we MUST NOT hardcode "Dhurandhar".
    if (!queryToSearch && !track) {
       // We couldn't decode it and there was no text.
       // Instead of sending random numbers or hardcoded names, we alert the user.
       alert("Could not read song name from the image. Since Spotify Codes cannot be decoded directly without a server, please ensure the song title is visible in the camera frame so it can be read.");
       setScanState('idle');
       return;
    }
